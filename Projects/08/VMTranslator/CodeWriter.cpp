...
CodeWriter::CodeWriter(const string& outPutFile, bool noMainFunction) : labelIndex(0), retIndex(0){
...
void CodeWriter::writeLabel(const string& label){
    vector<string> asmLines;
    if(currentFuncName.length()){
        asmLines.push_back("(" + currentFuncName + "$" + label + ")");
    }else{
...
void CodeWriter::writeFuncion(const string& functionName, int nArgs){
    ...
    asmLines.push_back("(" + functionName + ")");
    for(int i = 0; i < nArgs; ++i){
        asmLines.push_back("@SP"); 
        asmLines.push_back("A=M");
        asmLines.push_back("M=0");
        asmLines.push_back("@SP");
        asmLines.push_back("M=M+1");
    }   
    writeLines(asmLines);
...
void CodeWriter::writeCall(const string& functionName, int nArgs){
    ...
    asmLines.push_back("@" + currentFuncName + "$ret." + std::to_string(retIndex));
    asmLines.push_back("D=A");
    asmLines.push_back("@SP"); 
    asmLines.push_back("A=M");
    asmLines.push_back("M=D");
    ...
    asmLines.push_back("@SP");
    asmLines.push_back("D=M");
    asmLines.push_back("@5");
    asmLines.push_back("D=D-A");
    asmLines.push_back("@" + std::to_string(nArgs));
    asmLines.push_back("D=D-A");
    asmLines.push_back("@ARG");
    asmLines.push_back("M=D");
    ...
    writeLines({"(" + currentFuncName + "$ret." + std::to_string(retIndex++) + ")"});
...
void CodeWriter::writeReturn(){
    ...
    //gets the address at the frame’s end, then save it into R13
    asmLines.push_back("@LCL");
    asmLines.push_back("D=M");
    asmLines.push_back("@R13");
    asmLines.push_back("M=D");

    //gets the return address, then save it into R14
    asmLines.push_back("@5");
    asmLines.push_back("D=D-A");
    asmLines.push_back("A=D");
    asmLines.push_back("D=M");
    asmLines.push_back("@R14");
    asmLines.push_back("M=D");
